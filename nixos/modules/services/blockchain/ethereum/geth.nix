...
let
  ...
  gethOpts = { config, lib, name, ...}: {
    ...
    options = {
      ...
      network = mkOption {
        type = types.enum [ "mainnet" "goerli" "inkeby" "yolov2" "ropsten" ];
        default = "mainnet";
        description = ''
  ...
  config = mkIf (eachGeth != {}) {
    ...
    systemd.services = mapAttrs' (gethName: cfg: (
      ...
      serviceConfig = {
        DynamicUser = true;
        Restart = "always";
        StateDirectory = "goethereum/${gethName}/${cfg.network}";
      ...
      script = ''
        ${cfg.package}/bin/geth \
          --nousb \
          --ipcdisable \
          ${optionalString (cfg.network == "goerli") "--goerli"} \
          ${optionalString (cfg.network == "rinkeby") "--rinkeby"} \
          ${optionalString (cfg.network == "yolov2") "--yolov2"} \
          ${optionalString (cfg.network == "ropsten") "--ropsten"} \
          ...
          ${cfg.extraArgs} \
          --datadir /var/lib/goethereum/${gethName}/${cfg.network}
      '';
    }))) eachGeth;
